...
One approach I've never tried before is to store the list of FEN positions and solely on that list determine what move is possible. The list is needed to check the 50 move rule and the three fold repetition. (We only need the list from the last capture or pawn move.) But those could be seperate checks. First just take a FEN and tell me the possible moves. Input move via terminal and add the new position to the list of FEN's.
